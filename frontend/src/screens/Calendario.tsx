...
type Barber = components['schemas']['Barber'];
...
const Calendario = () => {
    ...
    // Security & Flow Check: Verify if user is logged in
    useEffect(() => {
        if (!isAuthenticated) {
            navigate('/login', { state: { from: '/calendario' } });
        }
    }, [isAuthenticated, navigate]);

    // Flow Check: Must have services selected
    useEffect(() => {
        if (selectedServicesIds.length === 0) {
            navigate('/seleccion');
        }
    }, [selectedServicesIds, navigate]);

    // Fetch Barbers
    useEffect(() => {
        const fetchBarbers = async () => {
            try {
                const res = await bookingService.getBarbers();
                const data = (res.data as any).results || res.data;
                setBarbers(data);
                setAllBarbers(data); // Sync with store
                
                if (!selectedBarber && data.length > 0) {
                    setBarber(data[0].id);
                }
            } catch (err) {
        ...
        fetchBarbers();
    }, [selectedBarber, setBarber, setAllBarbers]);
    ...
    useEffect(() => {
        const fetchAvailability = async () => {
            ...
            try {
                const date = selectedDate.dateObj;
                const formattedDate = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
                
                const res = await bookingService.getDisponibilidad(selectedBarber, formattedDate);
                setAvailableSlots(res.data.disponibles || []);
            } catch (err) {
    ...
    useEffect(() => {
        if (!selectedDate) {
            setDateAndTime(getNextDays(14)[0], null);
        }
    }, [selectedDate, setDateAndTime]);

    const setSelectedDateItem = (d: any) => setDateAndTime(d, null);
    const setSelectedTimeItem = (t: string) => setDateAndTime(selectedDate, t);
            ...
            <main className="pt-24 pb-48 px-6 max-w-4xl mx-auto">
                <section className="mb-12">
                    <button 
                        ...
                        className="flex items-center gap-2 text-on-surface-variant hover:text-[#F2CA50] transition-colors mb-6 group"
                    >
                        <span className="material-symbols-outlined transition-transform group-hover:-translate-x-1">arrow_back</span>
                        <span className="font-label text-xs tracking-widest uppercase">Volver a Servicios</span>
                    </button>
                ...
                <div className="space-y-12">
                    ...
                    <section>
                        ...
                        {loadingBarbers ? (
                            <div className="flex justify-center p-8 text-[#F2CA50]">Cargando especialistas...</div>
                        ) : (
                                ...
                                {barbers.map(barber => (
                                    ...
                                    >
                                        <div className="w-16 h-16 rounded-full overflow-hidden shrink-0 border-2 border-[#F2CA50]/20">
                                            <ChromaticImage 
                                                src={barber.foto_url || ''} 
                                                alt={barber.nombre_completo} 
                                                forceColor={selectedBarber === barber.id}
                                                className="w-full h-full object-cover" 
                                            />
                                        </div>
                                        <div>
                                            <h4 className="font-headline font-bold text-lg text-on-surface">{barber.nombre_completo}</h4>
                                            <p className="text-xs text-[#F2CA50] font-label tracking-widest uppercase">{barber.especialidad}</p>
                                        </div>
                                        {selectedBarber === barber.id && (
                    ...
                    <section>
                        ...
                        {loadingSlots ? (
                            <div className="flex justify-center p-8 text-[#F2CA50]">Consultando disponibilidad...</div>
                            ...
                            <div className="bg-surface-container-low p-8 rounded-xl text-center text-on-surface-variant italic">
                                No hay horarios disponibles para este día o profesional.
                            </div>
                        )}
                    </section>
                </div>
