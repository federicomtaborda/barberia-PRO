...
const Login = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // dj-rest-auth returns { access_token, refresh_token, user } or similar
            // based on settings.py it should be JWT
            ...
            if (access) {
                ...
                // If it returns 'key' (token auth) or other structure
                const token = response.data.key || response.data.token || response.data.access;
                setAuth(token, response.data.user || { email });
                navigate('/seleccion');
            }
        } catch (err: any) {
            ...
            <main className="relative z-10 w-full max-w-xl px-8 lg:px-20 flex flex-col justify-center min-h-screen animate-fade-in font-['Manrope']">
                ...
                <div className="mb-12">
                    <Link to="/" className="inline-flex items-center gap-3 active:scale-95 transition-transform group">
                        <span className="material-symbols-outlined text-[#F2CA50] group-hover:rotate-12 transition-transform">content_cut</span>
                        <span className="font-headline text-[#F2CA50] font-bold tracking-tighter text-2xl uppercase">THE ATELIER</span>
                    </Link>
                </div>
                ...
                <section className="space-y-8 bg-surface-container-low/40 backdrop-blur-md p-8 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-1 bg-[#F2CA50] opacity-30"></div>
                    ...
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        {/* Email Input Group */}
                        <div className="space-y-2">
                            <label className="block text-[10px] font-bold text-[#F2CA50] tracking-[0.2em] uppercase" htmlFor="email">Email</label>
                            <input
                                className="w-full h-14 px-5 bg-background border border-outline-variant/20 rounded-xl focus:border-[#F2CA50]/50 focus:ring-1 focus:ring-[#F2CA50]/20 text-on-surface font-body transition-all outline-none placeholder-neutral-700"
                                id="email"
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                <label className="block text-[10px] font-bold text-[#F2CA50] tracking-[0.2em] uppercase" htmlFor="password">Contraseña</label>
                                <Link className="text-[10px] font-bold text-on-surface-variant hover:text-[#F2CA50] transition-colors tracking-tighter" to="/">¿OLVIDASTE TU CONTRASEÑA?</Link>
                            </div>
                            <input
                                className="w-full h-14 px-5 bg-background border border-outline-variant/20 rounded-xl focus:border-[#F2CA50]/50 focus:ring-1 focus:ring-[#F2CA50]/20 text-on-surface font-body transition-all outline-none placeholder-neutral-700"
                                id="password"
                        ...
                        <div className="pt-4">
                            <button
                                disabled={loading}
                                className="w-full h-14 bg-[#F2CA50] text-neutral-950 font-headline font-bold text-lg rounded-xl tracking-tight shadow-xl shadow-[#F2CA50]/20 active:scale-95 hover:brightness-110 transition-all duration-400 cursor-pointer disabled:opacity-50"
                                type="submit"
                            >
                ...
                <footer className="mt-12 text-center">
                    <p className="text-on-surface-variant font-body text-sm">
                        ¿Nuevo en el club?{' '}
                        <Link className="text-[#F2CA50] font-bold hover:underline decoration-[#F2CA50]/30 underline-offset-4" to="/registro">Crea tu cuenta aquí</Link>
                    </p>
                </footer>
            </main>
        </div>
