...
const Registro = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await bookingService.register({
            ...
            // After registration, dj-rest-auth might return token or we might need to login
            // Usually returns { access, refresh, user } or { key, user }
            const { access, user, key } = response.data;
            const token = access || key;

            if (token) {
                setAuth(token, user || { email: formData.email, username: formData.username });
                navigate('/seleccion');
            } else {
                // If token is not returned, redirect to login
            ...
            if (errors) {
                // Handle complex error objects from DRF
                msg = Object.values(errors).flat().join(" ") || msg;
            }
    ...
    return (
        <div className="bg-background min-h-screen">
            <main className="min-h-screen flex flex-col md:flex-row">
                ...
                {/* Registration Side */}
                <section className="w-full md:w-1/2 flex items-center justify-center p-8 md:p-12 lg:p-20 overflow-y-auto">
                    <div className="w-full max-w-md space-y-10 animate-fade-in pt-20">
                        {/* Brand Anchor */}
                        ...
                        <form className="space-y-5" onSubmit={handleSubmit}>
                            {/* Username */}
                            <div className="space-y-2">
                                <label className="block text-[10px] font-bold text-primary tracking-[0.2em] uppercase">Usuario</label>
                                <input
                            ...
                            {/* Telefono */}
                            <div className="space-y-2">
                                <label className="block text-[10px] font-bold text-primary tracking-[0.2em] uppercase">Teléfono (Opcional)</label>
                                <input
                                    className="w-full h-14 px-5 bg-surface-container-low border border-outline-variant/20 rounded-xl focus:border-primary/50 focus:ring-1 focus:ring-primary/20 text-on-surface font-body transition-all outline-none placeholder-neutral-700"
                                    name="telefono"
                                    placeholder="+54 11 ..."
                            ...
                            <div className="space-y-6 pt-4">
                                <button
                                    disabled={loading}
                                    className="w-full h-14 editorial-gradient text-on-primary font-headline font-bold text-lg rounded-xl tracking-tight shadow-xl shadow-primary/20 active:scale-95 hover:brightness-110 transition-all duration-400 cursor-pointer disabled:opacity-50"
                                    type="submit"
                                >
